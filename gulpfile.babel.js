...
function scssCopy() {
  return gulp
    .src(paths.scssCopy) //개발코드 위치
    .pipe(gulp.dest(dist + "/assets/scss/")) // dist에 복사
    .pipe(connect.reload()); //변경되면 실시간 새로고침
}
...
const build = gulp.series(clean, gulp.parallel(js, font, image, scss, scssCopy, html, lib, watcher, server));
